...
public class PDAPIClient implements ClientModInitializer {
    ...
    public static void initEntries() {
        ...
        ItemGroupEvents.modifyEntriesEvent(PDCreativeTabs.SPECIALTY_DRINKS_TAB).register(stacks -> {
            Stream<Holder.Reference<SpecialtyDrink>> stream = Minecraft.getInstance().level.registryAccess().lookupOrThrow(PDRegistries.SPECIALITY_DRINK_KEY).listElements();
            stream.sorted(DrinkUtil.alphabetizer(SpecialtyDrink::languageKey)).map(SpecialtyDrink::getAsItem).forEach(stacks::accept);
        });
